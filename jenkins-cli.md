...
### validate If CLi works annd authenticate

```sh
java -jar jenkins-cli.jar -s http://localhost:8080 who-am-i
```

### CLI supported command.

```sh
java -jar jenkins-cli.jar -s http://localhost:8080 help
```

> Make sure you have `JENKINS_USER_ID=XXXX` and `JENKINS_API_TOKEN=XXXX` environment variable available where ever you are running this command with valid user and token details.
...
```
> Check How does the node.xml looks like on this repo.
